...
class stream_get extends getinfo {
	...
	function curl2($url, $cookies, $post, $header = 1, $json = 0, $ref = 0, $xml = 0, $h = NULL)
	{
		$ch = @curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		if ($json == 1) {
	...
	//	$linkdown = 'http://'.$sv_name.$hosting.'/'.$job['hash'].'/botken-'.urlencode($filename);
	//	$linkdown = 'http://'.$sv_name.'Bot-Ken-Hitpro/'.$hosting.'/'.$job['hash'].'/'.urlencode($filename);
	//	$linkdown = 'http://'.$sv_name.'Bot-Ken-Hitpro/Downloads.aspx?file='.$job['hash'].'/'.urlencode($filename);
...
class Tools_get extends getinfo {
	...
	function site_hash($url){
		...
		elseif (strpos($url, "uploaded.to") || strpos($url, "ul.to") || strpos($url, "uploaded.net")) $site = "ULD";
		...
		elseif (strpos($url, "uptobox.com"))   $site = "UTB";
		elseif (strpos($url, "debriditalia.com"))   $site = "ITALY";
		else {
			$schema = parse_url($url);
			$site = strtoupper(preg_replace("/(www\.|\.com|\.net|\.biz|\.info|\.org|\.us|\.vn|\.jp|\.fr|\.in|\.to)/","",$schema['host']));
